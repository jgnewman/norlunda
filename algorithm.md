...
>
> For this reason, you should translate individual lemmas first and only create compounds in the final phase of the algorithm. When using the automated algorithm, compounds that are assumed to be historical can be generated by placing a, **⟨:⟩** symbol between lemmas that should be combined, for example, **erþō:bazją**.
